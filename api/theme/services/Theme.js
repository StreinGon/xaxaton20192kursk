...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Theme
      .findOne(_.pick(params, _.keys(Theme.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count themes.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Theme.create(data);

    // Create relational data and return the entry.
    return Theme.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Theme.associations.map(a => a.alias));
    const data = _.omit(values, Theme.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Theme.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Theme.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Theme.attributes).reduce((acc, curr) => {
      switch (Theme.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
